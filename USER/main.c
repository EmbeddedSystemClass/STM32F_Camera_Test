...
int main(void)
{
	...
	SYS_Clock = HAL_RCC_GetSysClockFreq();
	
	while(mt9v034_context_configuration())
	{
		LCD_ShowxNum(30,150,version,16,16,0); //闪烁显示提示信息
		LCD_ShowxNum(30,170,version8_bit,16,16,0); //闪烁显示提示信息
		LCD_ShowString(30,130,240,16,16,"MT9V034 ERR");
		delay_ms(200);
	   // LCD_Fill(30,130,239,170,BLACK);
		delay_ms(200);
        LED0_Toggle;
	}
...
//        LED0_Toggle;
//	}	
    LCD_ShowString(30,130,200,16,16,"camera OK");
    ...
    while(1)
    {
       key=KEY_Scan(0);
		...
		t++; 									  
		if(t==100)LCD_ShowxNum(30,150,version,16,16,0); //闪烁显示提示信息
 		if(t==200)
		{	
